...
Status RceOptimizerVisitor::HandleReduce(HloInstruction* reduce) {
  ...
  if (Match(reduce, m::Reduce(m::Reshape(m::Op(&op)), m::Op(&init_value)))) {
    ...
    ss << "\n ---> reshape_shape=" << reshape_shape.ToString();
    ss << "\n ---> reduce_shape=" << reduce_shape.ToString();
    ss << "\n";
    ...
    // bool unchanged = true;
    // for(auto i = 0; i < optional_reshaped_indices.size(); i++){
    //   if( op->operand(0)->dimensions(i)!=op->dimensions(optional_reshaped_indices[i]))
    //   unchanged = false;
    // }
...
StatusOr<bool> RceOptimizer::Run(HloModule* module) {
  ...
  changed |= rce_change;
  // {
  //   LOG(INFO) << "Running subpipeline in RCE optimizer for " <<
  //   module->name() << "'"; HloPassPipeline
  //   subpipeline("before_rce_optimization_pipeline");
  //   //
